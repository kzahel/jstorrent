# DHT Phase 5: Bootstrap - Agent Task Document
...
Implement DHT bootstrap functionality to populate the routing table from well-known entry points. This is essential for a new DHT node to join the network - without bootstrap, the routing table is empty and peer discovery cannot function.

**Reference:** BEP 5 states: "Upon inserting the first node into its routing table and when starting up thereafter, the node should attempt to find the closest nodes in the DHT to itself. It does this by issuing find_node messages to closer and closer nodes until it cannot find any closer."

### Goals

1. Add well-known bootstrap node addresses to constants
2. Implement iterative `bootstrap()` method on `DHTNode`
3. Handle unresponsive nodes gracefully
4. Emit `bootstrapped` event when complete
5. Full test coverage for all bootstrap scenarios
...
├── constants.ts        # Add BOOTSTRAP_NODES array
└── dht-node.ts         # Add bootstrap() method and related types

packages/engine/test/dht/
└── dht-node-bootstrap.test.ts  # New test file
```

---

## Phase 5.1: Add Bootstrap Nodes to Constants

### File: `packages/engine/src/dht/constants.ts`

Add the following after the existing exports (after `MAX_NODE_ID`):
...
Add to the constants export section:
...
export {
  ...
  CLIENT_VERSION,
  MAX_NODE_ID,
  BOOTSTRAP_NODES,           // Add this
  BOOTSTRAP_CONCURRENCY,     // Add this
  BOOTSTRAP_MAX_ITERATIONS,  // Add this
} from './constants'
```

---

## Phase 5.2: Add Bootstrap Options Type

### File: `packages/engine/src/dht/dht-node.ts`

Add a new interface after `DHTNodeOptions`:
...
Update `DHTNodeEvents` interface to include the `bootstrapped` event:
...
## Phase 5.3: Implement Bootstrap Method

### File: `packages/engine/src/dht/dht-node.ts`

Update the existing imports to include the new constants and utilities:

**Update the constants import** (around line 29) from:
```typescript
...
To:
```typescript
import { NODE_ID_BYTES, K, BOOTSTRAP_NODES, BOOTSTRAP_CONCURRENCY, BOOTSTRAP_MAX_ITERATIONS } from './constants'
```

**Update the xor-distance import** (around line 28) from:
```typescript
...
Add the `bootstrap()` method after the existing outgoing query methods (after `announcePeer()`). Add it in a new section:
...
```typescript
  ...
  async bootstrap(options: BootstrapOptions = {}): Promise<BootstrapStats> {
    ...
    const queryNode = async (node: {
      ...
      try {
        const nodes = await this.findNode(node, this.nodeId)
        responsesReceived++
        return nodes
      } catch {
        failures++
...
## Phase 5.4: Export New Types

### File: `packages/engine/src/dht/index.ts`

Update the Phase 4 exports section:
...
export type {
  ...
  DHTNodeEvents,
  GetPeersResult,
  BootstrapOptions,    // Add this
  BootstrapStats,      // Add this
} from './dht-node'
...
## Phase 5.5: Create Bootstrap Tests

### File: `packages/engine/test/dht/dht-node-bootstrap.test.ts`
...
import { NODE_ID_BYTES, K } from '../../src/dht/constants'
import { generateRandomNodeId, nodeIdToHex, compareDistance } from '../../src/dht/xor-distance'
...
/**
 * Create a responder that returns progressively closer nodes.
 * Simulates realistic DHT behavior.
 */
function createProgressiveResponder(
  responderId: Uint8Array,
  allNodes: CompactNodeInfo[],
  targetId: Uint8Array,
): (query: any) => Uint8Array | null {
  return (query: any) => {
    if (query.q === 'find_node') {
      // Return K closest nodes to the target
      const sorted = [...allNodes].sort((a, b) => compareDistance(a.id, b.id, targetId))
      const closest = sorted.slice(0, K)
      return encodeFindNodeResponse(query.t, responderId, closest)
    }
    return null
  }
}
...
describe('DHTNode Bootstrap', () => {
  ...
  describe('options', () => {
    it('uses default BOOTSTRAP_NODES when none provided', async () => {
      // Import to check default values
      const { BOOTSTRAP_NODES } = await import('../../src/dht/constants')
...
```bash
# Run just the bootstrap tests
cd packages/engine
pnpm test -- dht-node-bootstrap
...
1. **Initialize**: Start with provided bootstrap nodes (or defaults)
2. **Query Phase**: Send `find_node(our_id)` to candidates
3. **Process Responses**: Add responding node to routing table, collect returned nodes
4. **Filter & Sort**: Remove already-queried nodes, sort by distance to self
5. **Iterate**: Pick closest unqueried nodes, repeat from step 2
6. **Terminate**: When no closer unqueried nodes exist or max iterations reached
7. **Report**: Emit `bootstrapped` event with statistics

### Key Design Decisions

1. **Parallel Queries**: Use `Promise.all()` with concurrency limit for faster bootstrap
2. **Deduplication**: Track queried nodes by `host:port` since we may not know node IDs initially
3. **Progressive Sorting**: As we learn node IDs, sort candidates by XOR distance to prioritize closer nodes
4. **Graceful Degradation**: Failures don't stop bootstrap; we continue with responsive nodes
5. **Bounded Iteration**: `maxIterations` prevents infinite loops in adversarial scenarios

### Relationship to Phase 6 (Iterative Lookup)

Bootstrap and iterative lookup share similar concepts:
- Both query nodes and process `find_node` responses
- Both iterate toward a target (self for bootstrap, infohash for lookup)
- Both handle parallel queries and timeouts

However, they differ:
- Bootstrap targets our own ID; lookup targets arbitrary infohashes
- Bootstrap uses `find_node`; lookup uses `get_peers`
- Lookup collects tokens for later `announce_peer`

Phase 6 will implement `iterative-lookup.ts` as a more general algorithm that can be used for peer discovery. The bootstrap implementation here is intentionally simpler since it only needs to find nodes close to ourselves.
...
| File | Change |
|------|--------|
| `src/dht/constants.ts` | Add `BOOTSTRAP_NODES`, `BOOTSTRAP_CONCURRENCY`, `BOOTSTRAP_MAX_ITERATIONS` |
| `src/dht/dht-node.ts` | Add `BootstrapOptions`, `BootstrapStats`, `bootstrap()` method, update `DHTNodeEvents` |
| `src/dht/index.ts` | Export new constants and types |
| `test/dht/dht-node-bootstrap.test.ts` | New test file with comprehensive bootstrap tests |

---

## Test Coverage Checklist

- [x] Sends find_node(self) to bootstrap nodes
- [x] Populates routing table from responses  
- [x] Iterates until no closer nodes found
- [x] Works with empty initial routing table
- [x] Tolerates unresponsive bootstrap nodes
- [x] Emits `bootstrapped` event when complete
- [x] Handles all bootstrap nodes unresponsive
- [x] Handles partial failures gracefully
- [x] Throws if node not started
- [x] Uses default bootstrap nodes
- [x] Respects `maxIterations` option
- [x] Respects `concurrency` option
- [x] Returns accurate statistics
- [x] Handles duplicate nodes
- [x] Handles empty bootstrap array
- [x] Handles self-referential responses
