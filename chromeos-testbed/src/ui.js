...
const TESTS = {
  ...
  intent_link_auto: {
    ...
    description: 'Programmatic link click (NO user gesture). Watch: Does it work? Does it create blank tab?',
    run: async () => {
      ...
      link.style.cssText = 'display: block; margin: 10px 0; padding: 10px; background: #ffffcc; border: 2px solid #ffcc00; border-radius: 4px; text-decoration: none; color: #000;';
  ...
  intent_link_manual: {
    ...
    run: async () => {
      ...
      link.style.cssText = 'display: block; margin: 10px 0; padding: 12px; background: #ffffcc; border: 3px solid #ff6600; border-radius: 4px; text-decoration: none; color: #000; font-weight: bold; cursor: pointer;';
  ...
  daemon_warm: {
    ...
    description: 'PREREQ: JSTorrent Android app must be running. Single request to measure baseline latency.',
    run: async () => {
      const url = 'http://100.115.92.2:7800/status';
  ...
  daemon_cold_start: {
    ...
    description: 'BEFORE RUNNING: Force stop JSTorrent in Android settings. Then run this to see if daemon auto-starts.',
    run: async () => {
      const url = 'http://100.115.92.2:7800/status';
      ...
      const poll = async () => {
        ...
        try {
          const response = await fetch(url, { signal: AbortSignal.timeout(2000) });
          const text = await response.text();
          log('daemon_cold_success', { attempt, elapsed, status: response.status, body: text.slice(0, 200) });
          return; // Stop on success
        } catch (e) {
  ...
  intent_wake_daemon: {
    ...
    description: 'BEFORE RUNNING: Force stop JSTorrent. Then run this to see if intent wakes the daemon.',
    run: async () => {
      ...
      try {
        ...
        try {
          ...
          const elapsed = Date.now() - start;
          const text = await response.text();
          log('intent_wake_daemon_alive', { elapsed, status: response.status, body: text.slice(0, 200) });
        } catch (e) {
          const elapsed = Date.now() - start;
