...
* **`rpc-info.json`**
  Authored **exclusively** by `jstorrent-native-host`. Stores discovery information for this Chrome profile.
...
* **jstorrent-link-handler**: to determine whether a native-host is running and which browser/extension profile it corresponds to.
* **Extensions or controllers**: to identify the correct native-host for the correct Chrome profile.
...
```
{
  ...
  "pid": 31257,
  "port": 33259,
  "token": "0283d8d6-66f2-4b9f-8000-cfede0c0db82",
  "started": 1764074065,
...
* **Profile-specific isolation:**
  The rpc-info file is tied to the Chrome profile that launched the native-host. Different profiles maintain separate files and download-root lists. Currently the rpc-info file contains
  the name of the chrome profile, e.g. `rpc-info-Default.json` but we would like to move to
  having a single discovery file in the future.
...
1. The handler reads `rpc-info.json`. (Looping over all `rpc-info-*.json`)

2. Attempts to communicate with native-host using stored token and current pid.

3. **If native-host is reachable**:

   * Forwards the magnet or file path to the extension via native-host RPC.

4. **If native-host is not reachable**:

   * Checks rpc-info for a valid browser field.
   * Launches that browser with:
