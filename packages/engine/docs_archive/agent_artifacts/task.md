...
- [x] Improve Logger Output Format
    - [x] Rename `defaultLogger` to `basicLogger`
    - [x] Implement smart `defaultLogger` with context prefix formatting
    - [x] Verify formatting with tests
- [x] Enable Logging for `BtEngine`
    - [x] Implement `ILoggableComponent` in `BtEngine`
    - [x] Initialize logger for `BtEngine` instance
    - [x] Fix lint errors and type issues
