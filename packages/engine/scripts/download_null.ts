...
import { NullFileSystem } from '../src/adapters/null'
...
async function main() {
  ...
  if (!magnetLink?.startsWith('magnet:')) {
    console.error('Usage: pnpm download-null "magnet:?xt=urn:btih:..." [port]')
    process.exit(1)
  }
  ...
  // Null storage - data is discarded after verification
  const storageRootManager = new StorageRootManager(() => new NullFileSystem())
  storageRootManager.addRoot({ key: 'null', label: 'Null', path: '/dev/null' })
  storageRootManager.setDefaultRoot('null')
