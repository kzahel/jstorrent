...
export class BtEngine extends EventEmitter {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  handleIncomingConnection(nativeSocket) {
  ...
  async addTorrent(magnetOrBuffer, options = {}) {
    ...
    let parsedTorrent
    let magnetLink
    let torrentFileBase64
    let magnetDisplayName
    ...
    const initComponents = async (infoBuffer, preParsed) => {
   ...
   */
  async initTorrentFromSavedMetadata(torrent, infoBuffer) {
    if (torrent.hasMetadata) return // Already initialized
