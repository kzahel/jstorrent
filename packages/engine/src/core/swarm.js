 ...
 */
export function isSuspiciousPort(port) {
  // Port 0 is invalid
 ...
 */
export function getPortScorePenalty(port) {
 ...
 */
export function detectAddressFamily(ip) {
 ...
 */
export function peerKey(ip, port) {
 ...
 */
export function normalizeAddress(ip, extractMappedIPv4 = true) {
  if (!ip.includes(':')) {
    // Plain IPv4
 ...
 */
export function parseCompactPeers(data, family) {
  const peers = []
...
export class Swarm extends EventEmitter {
  constructor(logger) {
    ...
    this.connectingKeys = new Set()
    // PeerId index for identity grouping
    this.peerIdIndex = new Map() // peerId hex → Set of address keys
  }
