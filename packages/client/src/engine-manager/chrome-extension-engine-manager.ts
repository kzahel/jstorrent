...
export class ChromeExtensionEngineManager implements IEngineManager {
  ...
  setRateLimits(downloadLimit: number, uploadLimit: number): void {
    if (!this.engine) {
      console.warn('[ChromeExtensionEngineManager] Cannot set rate limits: engine not initialized')
      return
    }
    this.engine.bandwidthTracker.setDownloadLimit(downloadLimit)
    this.engine.bandwidthTracker.setUploadLimit(uploadLimit)
    console.log(
      `[ChromeExtensionEngineManager] Rate limits set: download=${downloadLimit === 0 ? 'unlimited' : downloadLimit + ' B/s'}, upload=${uploadLimit === 0 ? 'unlimited' : uploadLimit + ' B/s'}`,
    )
  }
  ...
  setConnectionLimits(
    ...
    if (!this.engine) {
      console.warn(
        '[ChromeExtensionEngineManager] Cannot set connection limits: engine not initialized',
      )
      return
    }
    this.engine.setConnectionLimits(maxPeersPerTorrent, maxGlobalPeers, maxUploadSlots)
    console.log(
      `[ChromeExtensionEngineManager] Connection limits set: maxPeersPerTorrent=${maxPeersPerTorrent}, maxGlobalPeers=${maxGlobalPeers}, maxUploadSlots=${maxUploadSlots}`,
    )
  }
  ...
  setDaemonRateLimit(opsPerSecond: number, burstSize: number): void {
    if (!this.engine) {
      console.warn(
        '[ChromeExtensionEngineManager] Cannot set daemon rate limit: engine not initialized',
      )
      return
    }
    this.engine.setDaemonRateLimit(opsPerSecond, burstSize)
    console.log(
      `[ChromeExtensionEngineManager] Daemon rate limit set: ${opsPerSecond} ops/sec, burst=${burstSize}`,
    )
  }
  ...
  setEncryptionPolicy(policy: 'disabled' | 'allow' | 'prefer' | 'required'): void {
    if (!this.engine) {
      console.warn(
        '[ChromeExtensionEngineManager] Cannot set encryption policy: engine not initialized',
      )
      return
    }
    this.engine.setEncryptionPolicy(policy)
    console.log(`[ChromeExtensionEngineManager] Encryption policy set: ${policy}`)
  }
  ...
  async setUPnPEnabled(enabled: boolean): Promise<void> {
    if (!this.engine) {
      console.warn('[ChromeExtensionEngineManager] Cannot set UPnP: engine not initialized')
      return
    }
  ...
  async setDHTEnabled(enabled: boolean): Promise<void> {
    if (!this.engine) {
      console.warn('[ChromeExtensionEngineManager] Cannot set DHT: engine not initialized')
      return
    }
    await this.engine.setDHTEnabled(enabled)
    console.log(`[ChromeExtensionEngineManager] DHT ${enabled ? 'enabled' : 'disabled'}`)
  }
